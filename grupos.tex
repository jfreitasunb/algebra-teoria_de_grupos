...
\begin{teorema}
	...
	\begin{enumerate}
		\item $G$ \'e nilpotente.
		\item Existe um inteiro positivo $m$ tal que $Z_m(G) = G$.
		\item Existe um inteiro positivo $n$ tal que $\gamma_n(G) = \{1\}$.
	\end{enumerate}
\end{teorema}
\begin{prova}
	Primeiro provaremos que $(i) \Rightarrow (ii)$ e que $(i) \Rightarrow (iii)$. Para isso, suponha que $G$ \'e nilpotente. Assin existe uma s\'erie central
	\[
		\{1\} = G_0 \le G_1 \le \cdots \le G_n = G
	\]
	...
	\'E imediato verificar que $(ii) \Rightarrow (i)$. Mostremos ent\~ao que $(iii) \Rightarrow (i)$. Se $\gamma_m(G) = \{1\}$ para algum $m$ ent\~ao
	\[
...
\begin{teorema}
	...
	\begin{enumerate}
		\item $G$ \'e nilpotente.
		\item $G$ tem a propriedade do normalizador.
		\item Todo subgrupo de Sylow de $G$ \'e normal.
		\item $G$ \'e o produto direto de seus subgrupos de Sylow.
		\item Todo subgrupo de $G$ \'e subnormal.
		\item Todo subgrupo maximal de $G$ \'e normal.
	\end{enumerate}
	\begin{prova}
		Vamos mostrar primeiro que $(i) \Rightarrow (ii) \Rightarrow (iii) \Rightarrow (iv) \Rightarrow (i)$.
		\begin{itemize}
			\item $(i) \Rightarrow (ii)$ Feito na Proposi\c{c}\~ao \ref{propriedade_do_normalizador}.

			\item $(ii) \Rightarrow (iii)$ Sejam $K$ um $p$-subgrupo de Sylow de $G$ e $H = N_G(K)$. Se $K \ne G$, ent\~ao por $(ii)$, $H \subsetneq N_G(H)$. Mas pelo Corol\'ario \ref{normalizador_do_normalizador_de_Sylow} temos $N_G(N_G(K)) = N_G(K) = H$. Logo $H = G$ e assim $K \unlhd G$.

			\item $(iii) \Rightarrow (iv)$ Como todo subgrupo de Sylow de $G$ \'e normal, ent\~ao se $H_1$, \dots, $H_t$ s\~ao todos os subgrupos de Sylow de $G$ para primos distintos, ent\~ao $H_1\dots H_t$ \'e um subgrupo de $G$ de ordem igual  a $|G|$ e tal que $H_i \cap (H_1 \dots H_{i - 1}H_{i + 1}\dots H_t) = \{1\}$. Logo $G \cong H_1 \times \cdots H_t$, como quer{\'\i}amos.

			\item $(iv) \Rightarrow (i)$ Como cada $H_i$ \'e um $p$-grupo, ent\~ao cada $H_i$ \'e nilpotente, logo da Proposi\c{c}\~ao \ref{produto_direto_de_grupos_nilpotentes} segue que $G$ \'e nilpotente.
		\end{itemize}

		Mostremos agora que $(i) \Rightarrow (v) \Rightarrow (vi) \Rightarrow(i)$.
		\begin{itemize}
			\item $(i) \Rightarrow (v)$ Feito no Lema \ref{subgrupo_subnormal_em_grupo_nilpotente_finito}.

			\item $(v) \Rightarrow (vi)$ Seja $H$ um subgrupo maximal de $G$. Sabemos que $H$ \'e subnormal. Da{\'\i} $H \sub M \unlhd G$, o que contraria o fato de $H$ ser maximal. Logo $H \unlhd G$.

			\item $(vi) \Rightarrow (i)$ Para isso vamos mostrar que todo $p$-subgrupo de Sylow de $G$ \'e normal e da{\'\i} como $(iii) \Rightarrow (i)$ teremos o resultado desejado.
