...
\begin{enumerate}[label=({\alph*})]
  \item Sejam $a$, $b$, $i$, $j \in \{1, \dots, n\}$ distintos. Mostre que existe um 3-ciclo $\sigma$, envolvendo $a$ e $b$ e mais uma letra, tal que $\sigma (aij)\sigma^{-1} = (bak)$ para algum $k$. Conclua que $(aij) \in \ger{(abl) \mid l \in \{1, \dots, n\}\backslash \{a,b\}}$.
...
\questao Se $\sigma \in S_n$ fixa algum $n$, isto \'e, $\sigma(n) = n$, defina $\beta \in S_{n - 1}$ por $\beta(i) = \sigma(i)$ para todo $i \ne n$. Mostre que $(-1)^\beta = (-1)^\sigma$.
