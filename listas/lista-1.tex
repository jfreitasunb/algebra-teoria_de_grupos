...
\begin{enumerate}[label=({\alph*})]
  ...
  \item $(GL_n(\R), +)$: o conjutno das matrizes invert{\'\i}veis $n \times n$ com entraddas reais com o produto usual de matrizes. Este grupo \'e abeliano? (O resultado \'e verdadeiro se considerarmos matrizes sobre $\Q$ ou $\cmp$.)
\end{enumerate}
...
\questao Seja $\Z_n^* = \Z_n-\{\overline{0}\}$. O conjunto $(\Z_n^*, \otimes)$, onde para $\overline{a}$, $\overline{b} \in \Z_n$, $\overline{a} \oplus \overline{b} = \overline{a + b}$ \'e um grupo? Caso contr\'ario, qual a condi\c{c}\~ao sobre $n$ para que $(\Z_n^*, \otimes)$ seja grupo? 
