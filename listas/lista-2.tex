...
\questao \'E verdade que se $K \unlhd H \unlhd G$, ent\~ao $K \unlhd G$?
...
\questao Sejam $G$ um grupo e $\mathcal{I}_a : G \to G$, para $a \in G$ fixado, definido por $\mathcal{I}_a(x) = axa^{-1}$.
\begin{enumerate}[label=({\alph*})]
...
\begin{align*}
  \psi : (G, \cdot) &\to (\mathcal{I}(G),\comp)\\
  a &\mapsto \mathcal{I}_a.
\end{align*}
Por defini\c{c}\~ao, $\psi$ \'e uma fun\c{c}\~ao sobrejetora.
\begin{enumerate}[label=({\alph*})]
  \item Mostre que $\psi$ \'e um homomorfismo de grupos.
  \item Mostre que $\ker\psi = Z(G)$ e que $\mathcal{I}(G) \cong G /Z(G)$.
  \item Mostre que se $G$ n\~ao \'e abeliano, ent\~ao $\mathcal{I}(G)$ n\~ao \'e c{\'\i}clico.
\end{enumerate}

\questao Seja $G$ um grupo finito e sejam $K \le H \le G$. Mostre que
  \[
    [G:K] = [G:H][H:K].
  \]
